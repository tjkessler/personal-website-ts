...
/** All projects. `featured: true` items appear on home Selected Work (in array order). */
...
/** Full `/projects` industry & applied work list. */
export const industryProjects = projects.filter(
  (p) => p.category === "industry",
);

/** Full `/projects` software repositories list. */
export const packageProjects = projects.filter((p) => p.category === "package");
